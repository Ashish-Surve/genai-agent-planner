...
class TimeBlockRepository(BaseRepository[TimeBlockModel]):
    ...
    def find_by_date(self, target_date: date) -> list[TimeBlockModel]:
        ...
        try:
            ...
            blocks = (
                ...
                .filter(self.model.start_time >= start_of_day, self.model.start_time <= end_of_day)
    ...
    def find_by_date_range(self, start_date: date, end_date: date) -> list[TimeBlockModel]:
        ...
        try:
            ...
            blocks = (
                self.session.query(self.model)
                .filter(self.model.start_time >= start_dt, self.model.start_time <= end_dt)
                .order_by(self.model.start_time.asc())
                .all()
            )
    ...
    def find_conflicts(
        self, start_time: datetime, end_time: datetime, exclude_block_id: str | None = None
    ) -> list[TimeBlockModel]:
        """Find time blocks that conflict with the given time range."""
        try:
            # Blocks conflict if: start1 < end2 AND start2 < end1
            query = self.session.query(self.model).filter(
                self.model.start_time < end_time, self.model.end_time > start_time
            )
            ...
            self.logger.debug(f"Found {len(conflicts)} conflicts for {start_time} - {end_time}")
            return conflicts
    ...
    def find_by_block_type(
        self, block_type: str, start_date: date | None = None, end_date: date | None = None
    ) -> list[TimeBlockModel]:
        """Find time blocks by type, optionally within a date range."""
        try:
            query = self.session.query(self.model).filter(self.model.block_type == block_type)
    ...
    def find_current_and_upcoming(self, hours_ahead: int = 4) -> list[TimeBlockModel]:
        ...
        try:
            ...
            blocks = (
                self.session.query(self.model)
                .filter(self.model.end_time >= now, self.model.start_time <= future)
                .order_by(self.model.start_time.asc())
                .all()
            )

            self.logger.debug(
                f"Found {len(blocks)} current and upcoming blocks " f"(next {hours_ahead} hours)"
            )
            return blocks
    ...
    def find_with_tasks(self) -> list[TimeBlockModel]:
        ...
        try:
            blocks = self.session.query(self.model).options(joinedload(self.model.task)).all()
