...
from adhd_planner.models.enums import EnergyLevel, Priority, TaskStatus
from adhd_planner.models.task import Task, TaskCreate
from adhd_planner.models.time_block import TimeBlock
...
def test_task_deadline_validation():
    ...
    with pytest.raises(ValidationError, match="must be in the future"):
        Task(title="Test", estimated_duration_minutes=30, deadline=past_date)
...
def test_task_is_overdue():
    ...
    future_task = Task(
        title="Test", estimated_duration_minutes=30, deadline=datetime.utcnow() + timedelta(hours=1)
    )
    assert future_task.is_overdue is False

    # Task without deadline - not overdue
    no_deadline_task = Task(title="Test", estimated_duration_minutes=30)
    assert no_deadline_task.is_overdue is False


def test_task_duration_accuracy():
    """Test duration accuracy calculation."""
    task = Task(title="Test", estimated_duration_minutes=60, actual_duration_minutes=90)
...
def test_task_tags_unique():
    ...
    task = Task(title="Test", estimated_duration_minutes=30, tags=["work", "important", "work"])
...
def test_time_block_validation():
    ...
    block = TimeBlock(start_time=start, end_time=end, duration_minutes=60)
...
def test_time_block_end_before_start():
    ...
    with pytest.raises(ValidationError, match="must be after"):
        TimeBlock(start_time=start, end_time=end, duration_minutes=60)
...
def test_task_create_schema():
    ...
    task_data = TaskCreate(
        title="New task", estimated_duration_minutes=45, priority=Priority.URGENT, tags=["work"]
    )
