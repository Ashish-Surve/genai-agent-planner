...
def test_find_conflicts(block_repo):
    ...
    conflicts = block_repo.find_conflicts(datetime(2025, 1, 1, 9, 30), datetime(2025, 1, 1, 10, 30))
...
def test_find_conflicts_no_overlap(block_repo):
    ...
    conflicts = block_repo.find_conflicts(datetime(2025, 1, 1, 10, 0), datetime(2025, 1, 1, 11, 0))
...
def test_find_conflicts_exclude_block(block_repo):
    ...
    conflicts = block_repo.find_conflicts(
        datetime(2025, 1, 1, 9, 30), datetime(2025, 1, 1, 10, 30), exclude_block_id=block.id
    )
    assert len(conflicts) == 0
...
def test_find_by_block_type_with_date_range(block_repo):
    ...
    blocks = block_repo.find_by_block_type(
        "TASK", start_date=date(2025, 1, 1), end_date=date(2025, 1, 2)
    )
    assert len(blocks) == 1
